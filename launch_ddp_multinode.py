...
srun python cs336-systems/cs336_systems/naive_ddp_benchmarking.py --backend nccl --multinode --world_size {world_size} --model-config {model_config} --ddp-bucket --shard-optim
"""
...
def launch_slurm_jobs():
    ...
    for model_cfg in ["small", "medium", "large", "xl", "2.7B"]:
        name = f"ddp_sharded_benchmark_{model_cfg}"
